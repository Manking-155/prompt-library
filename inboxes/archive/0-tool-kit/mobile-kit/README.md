...
1) Keep only these folders: `agents/`, `workflows/`, `templates/`, `docs/`, and `mobile-kit-structure.md`. CLI/orchestrator and Python scripts are not required.
2) In Claude Code, open an agent file from `agents/` and use “Insert from file” to load it as system/context for the task.
3) To run a workflow, open a YAML in `workflows/` and paste it in a prompt; ask Claude to follow the steps sequentially.
4) When generating app code, paste the relevant template context (for example `templates/flutter/MOBILE_CONTEXT.md`) so Claude has the project constraints.
5) Optional: copy `docs/getting-started.md` and `docs/workflow-examples.md` content into Claude as additional context when needed.

Minimal example prompt to Claude Code:
```
Context: [paste templates/flutter/MOBILE_CONTEXT.md]
Role: [paste agents/mobile-planner.md]
Task: Plan and outline the user-profile feature for a Flutter app. Provide architecture, file list, and milestones.
Output: A concise plan with next actions.
```
